...
# Orphaned SGR remnants — after upstream code strips \x1b but leaves [XXm behind.
_ANSI_ORPHAN_RE = re.compile(r"\[[\d;]*m")

def _strip_ansi(text: str) -> str:
    """Remove all ANSI control + SGR sequences from *text*."""
    text = _ANSI_SGR_RE.sub("", _ANSI_CONTROL_RE.sub("", text))
    return _ANSI_ORPHAN_RE.sub("", text)
...
class ChatMessage:
    ...
    def __setattr__(self, name, value):
        if name in ('content', 'selected_label') and isinstance(value, str):
            value = _strip_ansi(value)
        elif name == 'choices' and isinstance(value, list):
            value = [(_strip_ansi(lbl) if isinstance(lbl, str) else lbl, val) for lbl, val in value]
...
def render_topbar(session_name: str, status: str, model: str, tasks_running: int,
    ...
    # LEFT: identity chip · session · status · global session dots
    left = Text()
    ...
    # Global session dots: show ALL sessions' status at a glance
    if sessions and len(sessions) > 1:
        left.append("  ", style=C_DIM)
        for sid, sess in sorted(sessions.items()):
            if sid == 0:
                continue  # skip if session id is 0 placeholder
            sess_st = getattr(sess, 'status', 'idle')
            needs_decision = not getattr(sess, 'ask_user_events', None) is None and not getattr(sess, 'ask_user_events', None).empty()
            if needs_decision:
                left.append("◆", style=f"bold {C_GREEN}")
            elif sess_st == "running":
                se = getattr(sess, '_busy_since', None)
                el = int(time.time() - se) if se else 0
                left.append("●", style=_heat_color(el))
            elif sess_st == "idle":
                dt = getattr(sess, '_done_at', None)
                if dt and (time.time() - dt) < 5:
                    left.append("●", style=f"bold {C_GREEN}")
                else:
                    left.append("○", style=C_DIM)
            else:
...
def render_bottombar(quit_armed: bool = False, rewind_armed: bool = False,
                    sessions_needing_decision: list = None) -> Table:
    t = Table.grid(expand=True)
    ...
    if quit_armed:
        ...
        # Decision-needed alert: highlight sessions awaiting user input
        if sessions_needing_decision:
            names = ", ".join(sessions_needing_decision[:3])
            suffix = f" +{len(sessions_needing_decision)-3}" if len(sessions_needing_decision) > 3 else ""
            left.append("◆ ", style=f"bold {C_GREEN}")
            left.append(f"需要决策: {names}{suffix}", style=f"bold {C_GREEN}")
            left.append("    ")
...
class GenericAgentTUI(App[None]):
    ...
    def _finalize_multi_choice(self, msg: ChatMessage, indices: list[int]) -> None:
        ...
        joined = _strip_ansi("; ".join(labels))
    ...
    def _collapse_choice(self, msg: ChatMessage, idx: int) -> None:
        ...
        display = _strip_ansi((result_text or label).strip() or label)
    ...
    def _refresh_topbar(self):
        ...
        self.query_one("#topbar", Static).update(
            render_topbar(s.name, s.status, model, tasks_running,
                          ...
                          term_width=term_w, sessions=self.sessions))
    ...
    def _refresh_bottombar(self):
        if not self.is_mounted: return
        # Collect sessions that need user decision
        needs_decision = []
        try:
            for sid, sess in self.sessions.items():
                aue = getattr(sess, 'ask_user_events', None)
                if aue is not None and not aue.empty():
                    n = (sess.name or "session").strip() or f"session-{sid}"
                    needs_decision.append(n)
        except Exception:
            pass
        try:
            self.query_one("#bottombar", Static).update(render_bottombar(
                ...
                sessions_needing_decision=needs_decision,
            ))
        except Exception:
            pass
    ...
    def _render_md(self, text: str, width: int):
        ...
        try:
            ...
            narrow_plain = _strip_ansi(narrow_raw)
    ...
    def _mount_message(self, container: VerticalScroll, m: ChatMessage) -> None:
        ...
        if m.kind in ("choice", "multi_choice") and m.selected_label is None:
            m._hint_widget = SelectableStatic(Text(_strip_ansi(m.content), style=C_MUTED), classes="msg")
        ...
        if m.kind in ("choice", "multi_choice"):  # selected_label is not None
            body = Text(); body.append("✓ ", style=C_GREEN); body.append(_strip_ansi(m.selected_label), style=C_FG)
            m._body_widget = SelectableStatic(body, classes="msg")
            container.mount(m._body_widget)
            return
        if m.role == "user":
            cleaned_user = _strip_ansi(m.content)
            body = Text(); body.append("> ", style=C_DIM); body.append(cleaned_user, style=C_FG)
            ...
            return
        if m.role == "system":
            m._body_widget = SelectableStatic(Text(_strip_ansi(m.content), style=C_MUTED), classes="msg")
            container.mount(m._body_widget)
            return
